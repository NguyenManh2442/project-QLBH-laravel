 ...
 <section id="data-thumb-view" class="data-thumb-view-header">
    ...
    <div class="card-content">
                    ...
                    <div class="col-xl-4 col-md-6 col-12 mb-1">
                        <fieldset class="form-group">
                            ...
                            <select class="select2 form-control" name="s_status">
                                <option></option>
                                <option value="0" {{ request()->s_status == 0 ? "selected" : "" }} >Ẩn</option>
                                <option value="1" {{ request()->s_status == 1 ? "selected" : "" }} >Hiển thị</option>
                            </select>
                        </fieldset>
                    </div>
                    <div class="col-xl-4 col-md-6 col-12 mb-1">
                        <fieldset class="form-group">
                            ...
                            <input type="text" class="form-control" name="s_supplier" value="{{ request()->s_supplier }}">
                        </fieldset>
                    </div>
                        ...
                        <fieldset class="form-group">
                            ...
                            <select class="select2 form-control" id="menucha" name="s_category1">
                                ...
                                @foreach($category as $key => $value)
                                    <option value="{{ $value->id }}" {{ request()->s_category1 == $value->id ? "selected" : "" }} >{{ $value->category_name }}</option>
                                @endforeach
                            </select>
                        </fieldset>
                    </div>
                            ...
                            <select class="select2 form-control" id="menucon" name="s_category">
                                @if (isset(request()->s_category))
                                    @foreach($subCategory as $key => $valueSubCategory)
                                        <option value="{{ $valueSubCategory->id }}" {{ $valueSubCategory->id ==  request()->s_category ? "selected" : "" }} >{{ $valueSubCategory->category_name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </fieldset>
     ...
     <div style="text-align: right; margin-top: 20px">
        <a href="/addProduct" class="btn bg-gradient-success mr-1 mb-1"><i class="feather icon-plus-square"></i> Add product</a>
     </div>
    <!-- dataTable starts -->
